      ...
      return (
        <Card className="bg-themeGray border-themeGray bg-clip-padding backdrop--blur__safari backdrop-filter backdrop-blur-2xl bg-opacity-60 p-1 lg:flex  md:rounded-xl flex items-center justify-center w-fit">
          <CardContent className="p-0 flex gap-2">
            {MALTIVASITY_CONSTANTS.groupPageMenu.map((menuItem) => (
              <Link
                href={menuItem.path}
                onClick={() => onSetSection(menuItem.path)}
                className={cn(
                  "rounded-xl flex gap-2 py-2 px-4 items-center hover:bg-themePurple",
          ...
          {MALTIVASITY_CONSTANTS.groupPageMenu.map((menuItem) => (
            <Link
              ...
              className={cn(
                ...
                section == menuItem.path ? "bg-themePurple border-[#27272A]" : "",
              )}
              key={menuItem.id}
            >
              {menuItem.icon}
